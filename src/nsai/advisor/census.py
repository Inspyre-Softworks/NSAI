"""Human-readable names for NationStates World Census scale IDs."""

from __future__ import annotations


CENSUS_SCALE_NAMES: dict[str, str] = {
    '0': 'Civil Rights',
    '1': 'Economy',
...
def census_scale_label(scale_id: object) -> str:
    """Return a readable census name while retaining the source ID."""

    normalized_id = str(scale_id).strip()
    name = CENSUS_SCALE_NAMES.get(normalized_id)
    if name:
        return f'{name} (#{normalized_id})'
    if normalized_id and normalized_id != '?':
        return f'Unknown stat (#{normalized_id})'
    return 'Unknown stat'


__all__ = ['CENSUS_SCALE_NAMES', 'census_scale_label']
